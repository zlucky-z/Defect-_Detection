...
class YOLOv5:
    def __init__(self, args):
        ...
        # 添加调试信息来确认模型输入尺寸
        print(f"Debug: 模型输入shape: {self.input_shape}")
        print(f"Debug: input_shape[2] = {self.input_shape[2]}, input_shape[3] = {self.input_shape[3]}")
        # 根据模型shape [1,3,1280,720]，正确设置网络输入尺寸
        self.net_h = self.input_shape[2]  # 1280
        self.net_w = self.input_shape[3]  # 720
        print(f"Debug: 设置后 net_h={self.net_h}, net_w={self.net_w}")
    ...
    def __call__(self, bmimg_list):
        ...
        if self.use_cpu_opt:
            ...
            print('self.postprocess type:', type(self.postprocess))
            print('DEBUG: non_max_suppression_custom in dir:', hasattr(self.postprocess, 'non_max_suppression_custom'))
...
def draw_bmcv(bmcv, bmimg, boxes, classes_ids=None, conf_scores=None, save_path=""):
    ...
    # 添加调试信息
    if len(boxes) > 0:
        print(f"Debug: 前3个检测框坐标:")
        for i in range(min(3, len(boxes))):
            x1, y1, x2, y2 = boxes[i, :].astype(np.int32).tolist()
            print(f"  框{i}: x1={x1}, y1={y1}, x2={x2}, y2={y2}, width={x2-x1}, height={y2-y1}")
    
    print("Debug: draw_bmcv 输入 boxes:", boxes)
    print("Debug: draw_bmcv 输入 conf_scores:", conf_scores)
    print("Debug: draw_bmcv 输入 classes_ids:", classes_ids)
    ...
    for idx in range(len(boxes)):
        ...
        if classes_ids is not None:
            ...
            # 将类别ID映射到0-1范围内，因为你只有2个类别
            mapped_class_id = class_id % 2
            color_index = mapped_class_id + 1  # 使用索引1和2
            color = np.array(COLORS[color_index]).astype(np.uint8).tolist()
        else:
            color = (0, 0, 255)
...
def main(args):
    ...
    if args.rtsp:
        # RTSP流模式
        logging.info("RTSP stream mode: {}".format(args.rtsp))
    ...
    # RTSP流处理
    if args.rtsp:
        logging.info("Starting RTSP stream detection...")
        decoder = sail.Decoder(args.rtsp, True, args.dev_id)
        if not decoder.is_opened():
            raise Exception("Cannot open RTSP stream: {}".format(args.rtsp))
        ...
        while not end_flag:
            ...
            if ret:  # 读取失败或流结束
                logging.warning("RTSP stream read failed or ended")
                end_flag = True
            else:
            ...
            if (len(frame_list) == batch_size or end_flag) and len(frame_list):
                ...
                for i, frame in enumerate(frame_list):
                    ...
                    # 每隔指定帧数保存一次检测结果
                    if cn % args.rtsp_save_interval == 0 or det.shape[0] > 0:
                        timestamp = time.strftime("%Y%m%d_%H%M%S", time.localtime())
                        save_path = os.path.join(output_img_dir, f'rtsp_frame_{cn}_{timestamp}.jpg')
                        
                        if det.shape[0] >= 1:
                            logging.info("Frame {}: detected {} objects".format(cn, det.shape[0]))
                            if args.use_cpu_opt:
                            ...
                            # 保存检测结果到JSON
                            res_dict = dict()
                            res_dict['frame_number'] = cn
                            res_dict['timestamp'] = timestamp
                            res_dict['image_name'] = f'rtsp_frame_{cn}_{timestamp}.jpg'
                            res_dict['bboxes'] = []
                            ...
                            # 即使没有检测到目标，也保存关键帧
                            if cn % args.rtsp_save_interval == 0:
                                bmcv.imwrite(save_path, frame_list[i])
                    
                    # 检查是否达到最大帧数限制
                    if args.rtsp_max_frames > 0 and cn >= args.rtsp_max_frames:
                        logging.info("Reached maximum frames limit: {}".format(args.rtsp_max_frames))
                        end_flag = True
                        break
        ...
        # 保存RTSP检测结果
        if results_list:
            json_name = f"rtsp_detection_results_{time.strftime('%Y%m%d_%H%M%S', time.localtime())}.json"
            with open(os.path.join(output_dir, json_name), 'w') as jf:
                json.dump(results_list, jf, indent=4, ensure_ascii=False)
            logging.info("RTSP detection results saved in {}".format(os.path.join(output_dir, json_name)))
        
        logging.info("RTSP stream processing completed. Total frames: {}".format(cn))
    ...
    # test images
        ...
        results_list = []
        cn = 0
        for root, dirs, filenames in os.walk(input_source):
        ...
        json_name = os.path.split(args.bmodel)[-1] + "_" + os.path.split(input_source)[-1] + "_bmcv" + "_python_result.json"
        with open(os.path.join(output_dir, json_name), 'w') as jf:
    ...
    # test video
        ...
        decoder = sail.Decoder(input_source, True, args.dev_id)
        if not decoder.is_opened():
            raise Exception("can not open the video")
        video_name = os.path.splitext(os.path.split(input_source)[1])[0]
        cn = 0
        frame_list = []
        end_flag = False
        while not end_flag:
...
def argsparser():
    ...
    parser.add_argument('--input', type=str, default='/data/pcb/dates/images/test.jpg', help='path of input (image/video file or directory)')
    parser.add_argument('--rtsp', type=str, default='', help='RTSP stream URL (e.g., rtsp://admin:password@192.168.1.100:554/stream)')
    parser.add_argument('--bmodel', type=str, default='/data/pcb/models/yolov5s_16848_f16.bmodel', help='path of bmodel')
    ...
    parser.add_argument('--rtsp_save_interval', type=int, default=30, help='save detection result every N frames for RTSP stream')
    parser.add_argument('--rtsp_max_frames', type=int, default=1000, help='maximum frames to process for RTSP stream (0 for unlimited)')
